...
function humanizeCvarKey(key: string, t: TFunction<"advanced">): string {
  const stripped = key.replace(/^(r\.|sg\.|fx\.)/, "");
  return stripped
    .split(".")
    .map((part) => {
      ...
      switch (lower) {
        case "max":
          return t("humanize.max", { defaultValue: "max" });
        case "min":
          return t("humanize.min", { defaultValue: "min" });
        case "quality":
          return t("humanize.quality", { defaultValue: "quality" });
        case "scale":
          return t("humanize.scale", { defaultValue: "scale" });
        case "distance":
          return t("humanize.distance", { defaultValue: "distance" });
        case "shadow":
          return t("humanize.shadow", { defaultValue: "shadows" });
        case "streaming":
          return t("humanize.streaming", { defaultValue: "streaming" });
        default:
...
export const ParameterRow = memo(function ParameterRow({
  ...
  return (
    <div
      className={cn(
        "group grid min-h-[58px] grid-cols-[24px_minmax(0,1fr)_minmax(180px,280px)_24px] items-center gap-3 border-b border-[var(--color-border)] bg-[var(--color-bg-soft)] px-3 py-2 transition hover:bg-[var(--color-surface)]",
    ...
    >
      <div className="flex justify-center">
        {engineToggleable && onEngineToggle ? (
          <Switch checked={engineEnabled ?? false} onChange={onEngineToggle} aria-label={engineEnabled ? t("inIni") : t("notInIni")} />
        ) : (
      ...
      <button
        type="button"
        className="grid h-7 w-7 place-items-center rounded-[var(--radius-control)] text-[var(--color-text-faint)] opacity-0 transition hover:bg-[var(--color-surface-hover)] hover:text-[var(--color-text-secondary)] group-hover:opacity-100"
