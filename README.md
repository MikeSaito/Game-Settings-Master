...
**02 — Авторские пресеты**  
Проверенные автором пресеты для некоторых игр (например Forza Horizon 6) — применение в один клик с предпросмотром diff. Видно каждую правку в конфигах.

**03 — Умная настройка**  
DLSS, FSR, ray tracing и Frame Generation — безопасный clamp под ваш GPU. Без бессмысленных опций на слабом железе.

**04 — Ручной редактор**  
Интерактивные ползунки, переключатели и списки для ключевых параметров UE4/UE5 — с описаниями, категориями и зависимостями.

**05 — Бэкапы**  
Snapshot перед каждым apply. Откат к предыдущему состоянию одним кликом — без страха сломать конфиг.

**06 — Авторский каталог пресетов**  
Каталог лежит в этом репозитории (`vps/public/`). Приложение подтягивает обновления с GitHub при запуске; без сети работает встроенная копия из установщика.

**07 — ReShade**  
Установка post-processing в папку игры: при запуске — только proxy DLL и базовый ini (без автопресетов). Вручную — пресеты Performance, Clarity, Cinematic и авторские ini для отдельных игр. Выбор API, запуск с ReShade или без.

## Скачать

Windows · бесплатно · без подписи издателя

* [Скачать установщик](https://github.com/MikeSaito/Game-Settings-Master/releases/latest/download/Game-Settings-Master_0.3.4_x64-setup.exe)
...
## Каталог авторских пресетов (разработка)

Исходники: `vps/source/`. Сборка и проверка:
...
Коммитьте `vps/public/` в `main` — приложение синхронизирует каталог с  
`raw.githubusercontent.com/MikeSaito/Game-Settings-Master/main/vps/public`.

Локальный preview: `cd vps && npm run dev` (http://localhost:8787).
